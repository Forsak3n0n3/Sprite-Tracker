...
export function exportCanvasImage(mode, baseSprites, filterSettings) {
    ...
    if (filterSettings.groupTheme) {
        targetItems = filters.sortByTheme(targetItems);
    }

    _renderCanvasExport(targetItems, titleL1, titleL2, fallbackTitleText, titleColor, fileName, mode, baseSprites, masteredSprites);
}

function _renderCanvasExport(targetItems, titleL1, titleL2, fallbackTitleText, titleColor, fileName, mode, baseSprites, masteredSprites) {
    ...
    mascotImg.src = 'siteimages/siteimage.png';
    
    mascotImg.onload = () => { _processRenderChain(); };
    mascotImg.onerror = () => { _processRenderChain(); };

    function _processRenderChain() {
        ...
        if (renderBars) {
            ...
            const colCount = baseSprites.filter(sprite => !sprite.unreleased && state.getObtainedSprites().includes(sprite.id)).length;
        ...
        targetItems.forEach((sprite, index) => {
            ...
            img.onload = () => {
                ...
                _drawSpriteCard(ctx, x, y, sprite, img, rarity, theme, isMastered, cardW, cardH, mode);

                loadedCount++;
                if (loadedCount === targetItems.length) {
                    _finalizeCanvas(canvas, footerLinkHeight, borderThickness, fileName);
                }
            };
...
function _drawSpriteCard(ctx, x, y, sprite, img, rarity, theme, isMastered, cardW, cardH, mode) {
...
function _finalizeCanvas(canvas, footerLinkHeight, borderThickness, fileName) {
    const ctx = canvas.getContext('2d');
    ...
    ctx.fillStyle = '#ffffff';
    let cleanUrlString = "Forsak3n0n3.github.io/Fortnite-Tracker/";
